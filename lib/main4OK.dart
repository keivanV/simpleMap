...
class _Task2ScreenState extends State<Task2Screen> {
  ...
  Future<void> _checkProximityToNewMarker(latlng.LatLng marker) async {
    ...
    if (distance <= 300 && mounted && !_notifiedMarkers.contains(index)) {
  ...
  Future<void> _checkProximityToMarkers() async {
    ...
    for (int i = 0; i < _markers.length; i++) {
      ...
      if (distance <= 300 && mounted && !_notifiedMarkers.contains(i)) {
  ...
  void _showHelpDialog() {
    showDialog(
      ...
      builder: (context) => Center(
        child: TaskHelpDialog(
          helpText:
              'شما می‌توانید بی‌نهایت نقطه در نقشه انتخاب کنید و هر زمان به شعاع ۳۰۰ متری این نقاط رسیدید، اعلان دریافت خواهید کرد. موقعیت مکانی شما در هر ۵ متر به‌روزرسانی خواهد شد.',
        ),
      ),
    );
  }
