  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: _isGpsDialogOpen ? null : _centerOnCurrentLocation,
    ...
    try {
      ...
      if (response.statusCode != 200) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'خطای API: ${response.statusCode}. لطفاً اتصال اینترنت را بررسی کنید.',
              ),
            ),
          );
        }
      ...
      if (data is Map && data.containsKey('error')) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'خطای API: ${data['error']['message']}. لطفاً نقاط دیگری انتخاب کنید.',
              ),
            ),
          );
        }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'خطا در دریافت مسیر ($vehicle): $e. لطفاً اتصال اینترنت را بررسی کنید.',
            ),
          ),
        );
      }
